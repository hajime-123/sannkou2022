...
def get_line_graph1_1(x,x2,x3,x4,x5):
    ...
    fig = figure(title="matmachine meand", x_axis_type="datetime",
                 tools=['pan','xwheel_zoom','ywheel_zoom','reset','save'],
...
def get_line_past1day(x,x1,x2,x3):
    file_name='./save2/1/'+x+'/'+'trig10_'+x1+'.csv'
    boxpd=pd.read_csv(file_name,engine='python',header=0)
    boxpd_x = pd.to_datetime(boxpd['time'])
    boxpd_y1 = boxpd['meand1']
    boxpd_y1_2 = boxpd['meand2']
    boxpd_y1_3 = boxpd['meand3']
    fig = figure(title="matmachine meand", x_axis_type="datetime",
                 tools=['pan','xwheel_zoom','ywheel_zoom','reset','save'],
                 y_range=(int(x2),int(x3)))
    #軸の設定
    format = "%Y-%m-%d-%H-%M-%S"
    fig.xaxis.formatter = DatetimeTickFormatter(
        seconds=[format],
...
def get_line_past1day2(x,x1,x2,x3):
    file_name='./save2/4/'+x+'/'+'trig05_'+x1+'.csv'
    boxpd=pd.read_csv(file_name,engine='python',header=0)
    boxpd_x = pd.to_datetime(boxpd['time'])
    boxpd_y1 = boxpd['rotate1']
    boxpd_y1_2 = boxpd['rotate2']
    boxpd_y1_3 = boxpd['rotate3']
    fig = figure(title="matmachine rotation", x_axis_type="datetime",
                 tools=['pan','xwheel_zoom','ywheel_zoom','reset','save'],
                 y_range=(int(x2),int(x3)))
    #軸の設定
