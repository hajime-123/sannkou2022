...
import dist2
app.register_blueprint(dist2.app)
import glob
import re
@app.route('/file1')
def root():
...
@app.route("/past_1day")
def past_page_1day():
    return render_template('past_page_1day.html')

@app.route('/past_postpage_1day', methods=['POST'])
def past_postpage_1day():
    text = request.json['t']
    text2 = request.json['t_2'].zfill(2)
    text3 = request.json['t_3'].zfill(2)
    date=text+text2#年月
    date1=text+text2+text3#年月日
    v1=request.json['v1']
    v2=request.json['v2']
    v3=request.json['v3']
    v4=request.json['v4']
    ...
    #---------
    try:
        plot =matflask_sub.get_line_past1day(date,date1,v1,v2)
        script, div = components(plot)
    except:
        return_data = {"result":'',
                       "result2":'',
        ...
        return jsonify(ResultSet=json.dumps(return_data))
    try:
        plot_2 =matflask_sub.get_line_past1day2(date,date1,v3,v4)
        script_2, div_2 = components(plot_2)
    except:
        return_data = {"result":script,
                       "result2":div,
...
@app.route('/save2/1/<test>/')
def show_file_1(test):
    files=glob.glob('save2/1/'+test+'/*')
    files=natsorted(files)
    html='<html><meta charset="utf-8"><body>'
    html+='<h1>ファイル一覧</h1>'
    for f in files:
...
@app.route('/save2/4/<test>/')
def show_file4_1(test):
    files=glob.glob('save2/4/'+test+'/*')
    files=natsorted(files)
    html='<html><meta charset="utf-8"><body>'
    html+='<h1>ファイル一覧</h1>'
    for f in files:
