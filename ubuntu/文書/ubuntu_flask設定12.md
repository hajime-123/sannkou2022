...
とりあえずデータを取得することができた。100が取得できた。

![1](ubuntu_flask設定画像12/2.PNG)

### PLCに対してPCから高速アクセスできるか確認

```
set HTTPS_PROXY=http://proxy.neg.co.jp:3128
...
![1](ubuntu_flask設定画像12/3.PNG)

一部ソフト変更

![1](ubuntu_flask設定画像12/4.PNG)

0.5秒ごとにも1秒ごとにもmat_colltestsub.func05()を入れるようにした。

![1](ubuntu_flask設定画像12/5.PNG)

0.5秒と1秒台にとれていることが確認できた。

使い勝手を良くするためにソフトを変更

![1](ubuntu_flask設定画像12/6.PNG)

![1](ubuntu_flask設定画像12/7.PNG)

CSV作る作業は共通なので関数化した。またリターンにはCSVの名前を当てた

今後データベースの際には戻りにオブジェクトを渡すことを考える

file_name~4までをもらうようにした

![1](ubuntu_flask設定画像12/8.PNG)

だめだった。todaydetailが更新されていなかった。

戻り値に追加しておく

![1](ubuntu_flask設定画像12/9.PNG)

mat_collmain.pyも一部変更

![1](ubuntu_flask設定画像12/10.PNG)

急にLANを抜いてみたが一応エラーは出力されそう

![1](ubuntu_flask設定画像12/11.PNG)

細かい話だがlogフォルダをlogsフォルダに変更した。
