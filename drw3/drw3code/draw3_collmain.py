...
#ファイルへ出力するハンドラーを定義
#when='D','H','M'
fh=logging.handlers.TimedRotatingFileHandler(filename='logs/drawlogmain.txt',
                                             when='H',
                                             backupCount=7)
# ログレベルを DEBUG に変更 デフォルトは値は warning info, debug はコンソールに出力されない
...
#test=mat_collsub.func_dbdel()
#logger.debug('テーブル数')#test20220119
#logger.debug(test)#test20220119
while True:
    start=time.time()
    ...
    if trig10_now!=trig10_pas:
        print('1.0秒',t3)
        #mat_log10.func1()
        try:
            draw3_collsub.func_10()
        except:
