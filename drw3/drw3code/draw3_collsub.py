# -*- coding: utf-8 -*-
"""
Created on Tue Jan 21 10:54:44 2020
...
#ファイルへ出力するハンドラーを定義
#when='D','H','M'
fh=logging.handlers.TimedRotatingFileHandler(filename='logs/drawlog.txt',
                                             when='H',
                                             backupCount=7)
# ログレベルを DEBUG に変更 デフォルトは値は warning info, debug はコンソールに出力されない
...
plc_send2=str("500000FF03FF000018000A04010000D*005000012C")#D5000から300個データを取得　ダブルワード
byte_data1=bytes.fromhex(plc_send1)
byte_data2=plc_send2.encode("UTF-8")
...
def func_dbcreate():
    ...
    `okuri_velocity` float DEFAULT NULL,\
    `hiki_velocity` float DEFAULT NULL,\
...
def csvcreate():
    ...
    if filesize==0:
        box1=['time','gaikei_value','naikei_value','okuri_velocity','hiki_velocity',\
...
def func_10():
    ...
    #(6)PLCの値をCSVに入れる
    box[0]=time1
    box[1]=func1_b(10,plc_ret1)/1000
    box[2]=func1_b(12,plc_ret1)/1000
    box[3]=func1_b(0,plc_ret1)/100
    box[4]=func1_b(2,plc_ret1)/100

    box[5]=func2_a(106,plc_ret2)/10
    box[6]=func2_a(116,plc_ret2)/10
    box[7]=func2_a(126,plc_ret2)/10
    box[8]=func2_a(136,plc_ret2)/10
    box[9]=func2_a(146,plc_ret2)/10
    box[10]=func2_a(156,plc_ret2)/10
    box[11]=func2_a(166,plc_ret2)/10
    box[12]=func2_a(176,plc_ret2)/10
    box[13]=func2_a(186,plc_ret2)/10
    box[14]=func2_a(196,plc_ret2)/10
    box[15]=func2_a(102,plc_ret2)/10
    box[16]=func2_a(112,plc_ret2)/10
    box[17]=func2_a(122,plc_ret2)/10
    box[18]=func2_a(132,plc_ret2)/10
    box[19]=func2_a(142,plc_ret2)/10
    box[20]=func2_a(152,plc_ret2)/10
    box[21]=func2_a(162,plc_ret2)/10
    box[22]=func2_a(172,plc_ret2)/10
    box[23]=func2_a(182,plc_ret2)/10
    box[24]=func2_a(192,plc_ret2)/10
    box[25]=func2_a(206,plc_ret2)/10
    box[26]=func2_a(216,plc_ret2)/10
    box[27]=func2_a(226,plc_ret2)/10
    box[28]=func2_a(236,plc_ret2)/10
    box[29]=func2_a(246,plc_ret2)/10
    box[30]=func2_a(256,plc_ret2)/10
    box[31]=func2_a(266,plc_ret2)/10
    box[32]=func2_a(276,plc_ret2)/10
    box[33]=func2_a(286,plc_ret2)/10
    box[34]=func2_a(296,plc_ret2)/10
    with open(file_name,mode,newline='')as file_obj:
        csv_writer=csv.writer(file_obj)
    ...
    try:
        ...
        #         %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,\
        #         %s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        sql_2 = '(`time`, gaikei_value, naikei_value, okuri_velocity, hiki_velocity,\
             H1PV, H2PV, H3PV, `H4-1PV`, `H4-2PV`, `H4-3PV`, `H4-4PV`, H5PV, H6PV, H7PV,\
             H1MV, H2MV, H3MV, `H4-1MV`, `H4-2MV`, `H4-3MV`, `H4-4MV`, H5MV, H6MV, H7MV,\
             roshinkann_PV1, roshinkann_PV2, roshinkann_PV3, roshinkann_PV4, roshinkann_PV5,\
             roshinkann_PV6, roshinkann_PV7, roshinkann_PV8, roshinkann_PV9, roshinkann_PV10)\
