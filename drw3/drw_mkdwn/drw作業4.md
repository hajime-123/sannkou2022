...
def sub_window2():
    ...
    if sub_win2 == None or not sub_win2.winfo_exists():
        ...
        #label_sub = tk.Label(sub_win, text="サブウィンドウ")
        #label_sub.pack()
        canvas = FigureCanvasTkAgg(fig, master=sub_win2)#(fig, master=root)
        ...
        def func_2():
            ...
            sql_1 = 'SELECT time,okuri_belocity,hiki_belocity from '+tablename10
            sql_2=' order by id desc limit %s'
            sql_3 = sql_1+sql_2
            data = (int(5),)
            data=b2_0
            if b2_0==None or  b2_0=='':            
                data = (int(5),)
            else:
                data=(int(b2_0),)
            ...
            #ax1.set_ylim(89990,90020)
            ax1.xaxis.set_major_formatter(mdates.DateFormatter('%y/%m/%d\n%H:%M:%S'))
            ax1.plot(boxpd_x,boxpd_y1,label="okuri_belocity")
            ax1.legend(loc='lower left')
            #Canvasのグラフに再描写する　1秒おきに以上を繰り返す
            fig.canvas.draw()
...
とりあえず表示はされたが、時刻が見にくかったり、グラフを自在に大きくしたりができない。まずはグラフの大きさを大きくできないか検討。
...
y1の値がmax、min二つとも0が入っているとエラーが出ていた

ただグラフを閉じてもエラーが出続けている？
...
グラフを落としてもtestが出力されつづけた。

root.afterの後の関数を止めないといけない。
